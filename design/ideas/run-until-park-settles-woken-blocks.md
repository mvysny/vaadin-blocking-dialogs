# `runUntilPark` also settles the blocks it woke

`runUntilPark` (`D_run_until_park`) returns once *its own* block parks or ends. SB-Emulators
needs one more thing: every block that its block *woke* — a dialog's parked block, resumed by the
OK click's block completing its future — must also have run up to its next park or end. Graduates
into `D_run_until_park` and `BlockingExecutor.runUntilPark`'s javadoc.

## Evidence (2026-09-24)

- `RunUntilParkProbeTest.aBlockWokenByTheCallIsNotSettledWhenItReturns`: block A parks on
  `answer`; `runUntilPark(() -> answer.complete("yes"))` returns with A still unresumed. A runs only
  at the next roundtrip. The library's own `RunUntilPark.returnsOnceTheBlockParks` shows the same
...
access task (`SessionCarrier.execute`). Under session-unlock the woken block is a worker thread,
...
blocks* goes to a scope-local queue instead of `session.access`. The caller mounts the first
...
**Session-unlock.** Each block woken inside the scope registers with it. The caller releases its
holds and waits until every registered block has parked or ended, then re-takes the holds — the
first-park wait of `D_input_exclusion`, over a set of blocks instead of one.

## Open questions

- `Q_scope_membership`: only submits made on a block's own thread while the scope runs, or also
...
- `Q_foreign_tasks`: SB-Emulators' drain also runs plain `ui.access` tasks queued during the
  cascade. The 2174-test run says nothing needs that beyond woken blocks. Confirm this after the
  change, with the drain removed.

## Side findings from the same review

- **Inline branch, stale UI after F5**: `RunUntilParkProbeTest.inlineParkAcrossF5`. The inner block
  resumes on the new UI, but the outer block, after `runUntilPark` returns, sees the **old, closed
  UI**: `runBlock`'s `restoreInstances(previous)` undoes `awaitAnchored`'s rebind.
...
- **Inline exceptions**: `D_run_until_park` cites `UI.accessSynchronously` as its naming model, but
  ...
  and continues the outer block (`exceptionsGoToTheErrorHandlerInlineToo`). SB-Emulators keeps
