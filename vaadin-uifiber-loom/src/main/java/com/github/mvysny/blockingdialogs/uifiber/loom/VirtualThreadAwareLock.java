 ...
 * fires nothing on purpose: the real outermost acquire/release is the carrier's, and a pretend hold
 * sits strictly inside that window.
 * <p>
...
public final class VirtualThreadAwareLock extends ReentrantLock {
    ...
    static VirtualThreadAwareLock asVirtualThreadAware(Lock sessionLock) {
        if (!(sessionLock instanceof VirtualThreadAwareLock)) {
            throw new IllegalStateException("Expected " + VirtualThreadAwareLock.class.getSimpleName()
                    ...
                    + LoomVaadinServlet.class.getSimpleName() + ", or route your VaadinService's"
                    + " getSessionLock() through VirtualThreadAwareLock.wrap()");
        }
     ...
     *                                       lock on behalf of the carrier and fail with
