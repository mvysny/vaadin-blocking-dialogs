 ...
 * session lock - the API routes a lock-less caller through {@code session.access} - so the fiber
 * resumes only once the completing thread lets go.
 *
...
public interface Completable<R> {
     ...
     * returns or throws. Outside {@link UIFiberRunnerSpi#runUntilFirstPark} the fiber's changes so far
     * reach the browser by the time it waits (the app has {@code @Push}); inside, the caller's response
     * carries them. Already completed, it returns at once and releases nothing - no park at all, for
     * {@link UIFiberRunnerSpi#runUntilFirstPark} neither.
     *
